<%@ page import="it.unibo.sdls.sampleproject.jmx.*" %>
<%@ page import="it.unibo.sdls.sampleproject.dao.*" %>
<%@ page import="java.util.*" %>
<%@ page import="it.unibo.sdls.sampleproject.dao.jdbc.pooled.mysql.PooledMySqlJDBCDAOFactory" %>
...
<%
    ...
    ObjectName name = new ObjectName("it.unibo.sdls.sampleproject.jmx:type=ManagedDAOFactory");

    // Crea istanza dell' MBean
    ManagedDAOFactory mdf = new ManagedDAOFactory(application);

    // Registra l’istanza
    if (mbs.isRegistered(name)) {
        mbs.unregisterMBean(name);
    }
    mbs.registerMBean(mdf, name);

    DAOFactory daoFactory = DAOFactory.getDAOFactory((String) application.getAttribute("daoFactory"));
...
<div>
    ...
    <form>
        ...
        %>
        <option value="<%= publisher.getId() %>"><%= publisher.getName()%>
        </option>
        <%
            }// end while
        %>
